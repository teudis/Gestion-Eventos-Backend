...
$("#edit_pack_special").on('click','button#listo_price_pack_editar',function(event){
        ...
        var id = rowCount - 1;
  		var name_pack = "edit_pack"+ id;  		
  		name_pack = $('#'+ name_pack).val();
  		var name_special = 'edit_special'+ id;
		  ...
		  var input_max = "<td>"+  name_special  + "</td>"
		   ...
		   var count_element = $("#edit_rows_special").val();
  		 var row =  "<tr>" + input_min + input_max + 
                    ...
                    + '</td>' + '</tr>'
	        $("#edit_pack_special tr").eq(rowIndex_pack).remove();
	          var pos = rowIndex_pack;
	         var ubication = "edit_pack_special tr:eq(" + pos + ")";
	        //$('#prices_age tr:eq(0)').after(row);
	        $('#'+ubication).before(row);
	        // add input 

	        name_pack_id = "edit_pack" + id;
	        name_special_id = 'edit_special' + id;
	         ...
	         if(  count_element >= rowIndex_pack)
		   {
				 ...
				 var ubication = "edit_prices_pack_hidden :eq(" + pos + ")";
				 //$('#prices_age tr:eq(0)').after(row);
				  $('#'+ubication).before('<p>' + input_min + input_max  + '</p>');
	       		...
	       		rowCount = $('#edit_pack_special tr').length;
  		   		$("#edit_rows_special").val(rowCount-2);
	...
	$("#edit_pack_special").on('click','button#editar_price_pack_editar',function(event){


        event.preventDefault(); 
        var rowIndex_pack_edit = ($(this).closest('td').parent()[0].sectionRowIndex);
        $("#edit_pack_special tr").eq(rowIndex_pack_edit).remove();
        // create tr

  var name_pack = 'edit_pack'+ rowIndex_pack_edit;
  var name_special = 'edit_special'+ rowIndex_pack_edit;
 ...
 //$('#prices_age tr:eq(0)').after(row);
  $('#'+ubication).before(row_pack);       
	...
	$("#edit_pack_special").on('click','button#editar_eliminar_price_pack',function(event){


        event.preventDefault();        
        var rowIndex2 = ($(this).closest('td').parent()[0].sectionRowIndex);
        $("#edit_pack_special tr").eq(rowIndex2).remove();
